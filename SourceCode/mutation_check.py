...
pro_mutation_set = [0.1*i for i in range(6)] # The probability of executing mutation
...
max_interval = 200 # The maximum interval between old and new best-so-far results.
...
def gen_algo():
    ...
    original_pop = gen_pop(popSize=size_pop, distM=distM, radius=r, K=K);
    original_pop.sort(reverse=True)
    for pro_mutation in pro_mutation_set:
        opt_fitness = [];
        time_cost = [];
        finish_generaiton = [];
        for iter in range(20):
            pop = deepcopy(original_pop)
            # time counter
            start = process_time()
            ...
            while max_interval > interval:
                ...
                if OPT < pop[0].fitness:
                    ...
                    OPT_time_cost = process_time() - start
                    # print("=======" * 20)
                    # print(f"Generation: {round}")
                    # print(f"The best-so-far fitness: {pop[0].fitness}")
                    # print(f"Time cost: {OPT_time_cost}")
                    # print(f"interval: {interval}")
                    OPT_interval= interval
                    interval = 0
            opt_fitness.append(pop[0].fitness);
            time_cost.append(OPT_time_cost);
            finish_generaiton.append(round);
        print("=======" * 10 + f"mutation_prob={pro_mutation}"+ "=======" * 10 )
        print(f"average # of generation: {mean(finish_generaiton)}")
        print(f"average opt fitness: {mean(opt_fitness)}")
        print(f"average time cost: {mean(time_cost)}")
